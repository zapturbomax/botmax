# Prompt 2: Layout Principal em Tela Cheia (FlowBuilderLayout.tsx)

Este componente é o principal do Flow Builder, ocupando toda a tela sem barras laterais, exatamente como no dispara.ai.

```tsx
// src/components/flow-builder/FlowBuilderLayout.tsx
...
1. **Layout em Tela Cheia**: Ocupa toda a área disponível com `h-screen w-screen`
2. **Barra Superior**: Contém botões para voltar, salvar, testar e configurações
3. **Área Principal Dividida em 3 Partes**:
   - Painel de propriedades à esquerda (visível apenas quando um nó está selecionado)
   - Canvas central (área de trabalho principal)
   - Painel de componentes à direita (sempre visível)
4. **Tooltips**: Para melhorar a usabilidade e explicar a função de cada botão

Este componente é o container principal que organiza todos os outros componentes do Flow Builder, garantindo um layout idêntico ao do dispara.ai.

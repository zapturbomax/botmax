# Prompt 8: Implementação dos Nós Específicos - Parte 3 (ConditionNode.tsx)

Este componente implementa o nó de condição, que permite criar ramificações no fluxo com base em condições específicas.

```tsx
// src/components/flow-builder/node-elements/ConditionNode.tsx
...
1. **Condições Dinâmicas**: Adicionar, remover e editar condições diretamente no nó
2. **Tipos de Condição**: Suporte para diferentes tipos de condição (começa com, contém, igual a)
3. **Condição Padrão**: Sempre inclui uma condição padrão que é executada se nenhuma outra condição for atendida
4. **Handles Múltiplos**: Um handle de saída para cada condição, permitindo fluxos diferentes
5. **Validação**: Impede remover a condição padrão ou ficar com menos de duas condições

Este componente representa um bloco de condição no fluxo de conversação, permitindo criar ramificações com base no conteúdo das mensagens recebidas do usuário.
